...
VARIANTS = {
    ...
    'preview-low-budget-patrol': {
        'revision': 'data-v16.9-preview-low-budget-patrol',
        'public_version': 'v16.9-preview',
        'name': 'Enemy Spawn Multiplier Preview Low Budget Fast Cadence',
        'description': 'Fast-cadence preview: forces the effective Encounter budget to the scaled director base instead of scaling a possibly positive native override, clamps the enemy reinforcement cooldown at director+0x399D8 to two seconds, drives timed Patrol/Straggler intervals to 0.0-0.1 seconds with the group clamp at 10x, and leaves GuardForce/static defenders on the native budget and schedule. Logs the actual override plus read-only scheduler counters. Native template weights, population gates and executable code remain unchanged. Requires the official Bingus Shared Loader v15 or newer.',
        'template_bias': False,
        'overrides': {
            ...
            'traveler_cooldown_enabled': True,
            'traveler_cooldown_min': 2.0,
            'traveler_cooldown_max': 5.0,
...
def main():
    ...
    report = {
        ...
        'version': 169 if key == 'preview-low-budget-patrol' else 16,
        ...
        'data_change': {
            ...
            'encounter_deadline_writer_rva': '0x94CCE8',
            'encounter_admission_rva': '0x94C030',
            'scheduler_a_rva': '0x276C2B0', 'scheduler_a_offset': '0x4A4',
            'scheduler_b_rva': '0x276CA28', 'scheduler_b_offset': '0x1C',
            'scheduler_flags': ['0x5189C', '0x518A0', '0x518A4', '0x518A8'],
            'hive_mind_config': {'component': 'HiveMindComponent', 'row_size': '0x438',
