...
VARIANTS = {
    ...
    'preview-low-budget-patrol': {
        'revision': 'data-v16.6-preview-low-budget-patrol',
        'public_version': 'v16.6-preview',
        'name': 'Enemy Spawn Multiplier Preview Low Budget Patrol Strict',
        'description': 'Strict low-budget preview: forces the effective Encounter budget to the scaled director base instead of scaling a possibly positive native override, drives timed Patrol/Straggler intervals to 0.0-0.1 seconds with the group clamp at 10x, and leaves GuardForce/static defenders on the native budget and schedule. Logs the actual override and the read-only Encounter cooldown deadline. Native template weights, population gates and executable code remain unchanged. Requires the official Bingus Shared Loader v15 or newer.',
        'template_bias': False,
        'overrides': {
...
def main():
    ...
    report = {
        ...
        'version': 166 if key == 'preview-low-budget-patrol' else 16,
