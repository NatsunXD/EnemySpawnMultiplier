...
The builder verifies the supported EXE and game.dll hashes, compiles both Lua variants, runs synthetic memory checks, and inspects each archive in Python. It writes `releases/Enemy-Spawn-Multiplier-6x-Native-Composition-v16.zip` and `releases/Enemy-Spawn-Multiplier-6x-Light-Medium-Bias-v16.zip`. Pass `base` or `light-medium` to build only one variant. It does not install or launch the game.
