...
The builder verifies the supported EXE and game.dll hashes, compiles all configured Lua variants, runs synthetic memory checks, and inspects each archive in Python. It writes `releases/Enemy-Spawn-Multiplier-6x-Native-Composition-v16.zip`, `releases/Enemy-Spawn-Multiplier-6x-Light-Medium-Bias-v16.zip`, and `releases/Enemy-Spawn-Multiplier-Preview-Low-Budget-Fast-Cadence-v16.8-preview.zip`. Pass `base`, `light-medium`, or `preview-low-budget-patrol` to build only one variant. It does not install or launch the game.
