...
- `Preview Low Budget Fast Cadence`：遭遇预算降至原版 0.1x 且强制正值 override 使用缩放后的基础预算；敌人增援冷却钳制到 2 秒；巡逻/散兵刷新间隔改为 0.0-0.1 秒，组规模上限保持 10x，守军预算和刷新机制保持原版
...
3. 在 HDArsenal 或 HD2MM 中导入 `Enemy-Spawn-Multiplier-6x-Native-Composition-v16.zip`、`Enemy-Spawn-Multiplier-6x-Light-Medium-Bias-v16.zip` 或 `Enemy-Spawn-Multiplier-Preview-Low-Budget-Fast-Cadence-v16.7-preview.zip`，三选一。
4. 重新部署并重启游戏。
...
Install the official [Bingus Shared Loader v15 or newer](https://github.com/CowboyBingus/BingusSharedLoader/releases/tag/v15), then install exactly one of `Enemy-Spawn-Multiplier-6x-Native-Composition-v16.zip`, `Enemy-Spawn-Multiplier-6x-Light-Medium-Bias-v16.zip`, or `Enemy-Spawn-Multiplier-Preview-Low-Budget-Fast-Cadence-v16.7-preview.zip`. Loader v15 discovers the declared entry automatically; no registry edit or custom loader fork is required.

A fast-cadence preview package tests the opposite density split: `Enemy-Spawn-Multiplier-Preview-Low-Budget-Fast-Cadence-v16.7-preview.zip` lowers the reinforcement budget to `0.1x` while forcing a positive `cfg+0x78` override to the scaled director base, clamps the enemy reinforcement cooldown at `director+0x399D8` to two seconds, runs timed Patrol/Straggler intervals at `0.0-0.1` seconds, keeps the `10x` cap and group expansion, and leaves GuardForce/static defenders native. The log records the cooldown delta, the two scheduling counters that gate the timed scheduler, the Patrol/Straggler deadline deltas and the scheduler enable flags. It uses the same resource ID, so install it instead of either v16 variant.
