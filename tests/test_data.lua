...
assert(patch.apply(api, game))
mission(vanilla, 100, 50)
fill_config(4, 8, 8 / 5, 14 / 5, 50, 30)
ok, reason, active = patch.apply(api, game)
assert(ok and active)
assert_config(4, 8, 8 / 5, 14 / 5, 50, 30)
...
assert(get_u32(selected + 0x48) == 50 and approx(get_f32(selected + 0x38), 4))
...
assert(get_u32(selected + 0x48) == 50 and approx(get_f32(selected + 0x40), 1.6))
assert(ffi.string(director + patch.cfg_base_offset, patch.cfg_stride) == decoy)
...
assert(get_u32(cloned_entries + patch.max_offset) == 40)
assert(get_u32(cloned_entries + patch.entry_stride + patch.max_offset) == 35)
assert(get_u32(cloned_entries + 2 * patch.entry_stride + patch.max_offset) == 50)
assert(get_f32(director + patch.points_offset) == 600)
assert(get_f32(director + patch.points_offset + 4) == 50)
